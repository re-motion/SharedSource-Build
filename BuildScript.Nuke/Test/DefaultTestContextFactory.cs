...
public class DefaultTestContextFactory : ITestContextFactory
{
  public ITestContext CreateTestContext (
      ITest build,
      ImmutableDictionary<string, string> testParameters,
      ImmutableArray<ITestResource> testResources)
  {
    return new DefaultTestContext(build, testParameters, testResources);
  }
}