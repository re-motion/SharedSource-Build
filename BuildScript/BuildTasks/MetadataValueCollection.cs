...
{
  /// <summary>
  /// Wraps a <see cref="IReadOnlyCollection{T}"/> and performs case-insensitive access operations, emulating default msbuild behavior.
  /// </summary>
  public class MetadataValueCollection
  {
    ...
    public string Single (IEnumerable<string> possibleValues, Func<Exception> createEmptySequenceException)
    {
      ...
      if (intersection.Length > 1)
      {
        ...
        throw new InvalidOperationException ($"Found more than one entry in the collection matching the set of possible values. Values: {jointIntersection}");
      }
    ...
    {
      return _rawCollection.Contains (value);
    }

    public bool IsEmpty ()
    {
      return !_rawCollection.Any();
    }
  }
}